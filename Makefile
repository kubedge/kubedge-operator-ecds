...
generate: setup
	...
	go run vendor/k8s.io/code-generator/cmd/deepcopy-gen/main.go --input-dirs github.com/kubedge/kubedge-operator-ecds/pkg/apis/kubedgeoperators/v1alpha1 -O zz_generated.deepcopy --bounding-dirs github.com/kubedge/kubedge-operator-ecds/pkg/apis
...
install-dev: docker-build-dev
	helm install --name kubedge-ecds-operator chart --set images.tags.operator=${IMG_DEV} --namespace ${K8S_NAMESPACE}

install-amd64:
	helm install --name kubedge-ecds-operator chart --set images.tags.operator=${IMG_AMD64},images.pull_policy=Always --namespace ${K8S_NAMESPACE}

install-arm32v7:
	helm install --name kubedge-ecds-operator chart --set images.tags.operator=${IMG_ARM32V7},images.pull_policy=Always --namespace ${K8S_NAMESPACE}

install-arm64v8:
	helm install --name kubedge-ecds-operator chart --set images.tags.operator=${IMG_ARM64V8},images.pull_policy=Always --namespace ${K8S_NAMESPACE}

purge: setup
	helm delete --purge kubedge-ecds-operator
